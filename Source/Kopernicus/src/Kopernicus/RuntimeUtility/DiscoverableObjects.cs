        ...
        private void Start()
        {
            ...
            else if (RuntimeUtility.KopernicusConfig.UseKopernicusAsteroidSystem.ToLower().Equals("stock"))
            {
                ...
                if (!DidWeSpawnThatRock)
                {
                    try
                    {
                        if (DiscoverableObjects.Asteroids.ToArray()[1].Name.Length > 1) //arbitrary test
                        {
                    ...
                    catch
                    {
                        ...
                        ConfigNode discoveryNode = ProtoVessel.CreateDiscoveryNode(DiscoveryLevels.StateVectors, UntrackedObjectClass.I, 999, 999);
